...
_PW: Optional[playwright.async_api.Playwright] = None
_BROWSER: Optional[playwright.async_api.Browser] = None
_PAGE_SEM = asyncio.Semaphore(4)

async def _get_browser() -> playwright.async_api.Browser:
    global _PW, _BROWSER
    if _BROWSER is None:
        _PW = await playwright.async_api.async_playwright().start()
        _BROWSER = await _PW.chromium.launch(headless=True, chromium_sandbox=True)
    return _BROWSER
...
async def screenshoot(id: int, output_path: str):
    ...
    async with _PAGE_SEM:
        ...
        async def _route_handler(route: playwright.async_api.Route):
            url = route.request.url.lower()
            if url.startswith(("http://", "https://")) or url.endswith((".woff",".woff2",".ttf",".otf",".map")):
        ...
        H = await page.evaluate("""
            () => {
                const doc = document.documentElement, bod = document.body;
                ...
                if (el && h <= 1500) el.style.height = (h - 100) + 'px';
                return h;
            }
        """)
        H = int(H)
        ...
        out = Image.new("RGBA", (chunks[0].width, total_h), (0,0,0,0))
        off = 0
        for im in chunks:
            out.paste(im, (0, off))
            off += im.height
            im.close()
        out.save(output_path, format="PNG", optimize=False, compress_level=6)
        out.close()
